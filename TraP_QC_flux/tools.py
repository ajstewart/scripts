...
#
    dump_images(database, username, password, dataset_id, databaseType, host, port)
#    if release == '1m':
...
def sidetime(target_ra, target_dec, mjd2, telescope):
#       Time is in mjd
#       This is the IRTF position of CS002
#       Returns both the elevation and the sidereal time for a given position
        if telescope=='LOFAR':
            x, y, z = (3826577.066110000, 461022.947639000, 5064892.786)
        elif telescope=='MWA':
            x,y,z=(-2556629.7620, 5097138.2540, -2848385.1050)
        dm = measures()
        position=dm.position("ITRF", "%fm" % (x,), "%fm" % (y,), "%fm" % (z,))
       # dm.do_frame(dm.position("ITRF", "%fm" % (x,), "%fm" % (y,), "%fm" % (z,)))
        zenith = dm.direction("AZEL", "0deg", "90deg")
        time = dm.epoch("UTC", quantity("%fd" % (mjd2,)))
       # dm.do_frame(time)
        target = dm.direction("J2000", "%fdeg" % (float(target_ra),), "%fdeg" % (float(target_dec),))
        
        targetAZEL = radec_to_azel(target,time,position)

#        print zenith['m1']['value'], targetAZEL['m1']['value'], 90.-math.degrees(zenith['m1']['value']-targetAZEL['m1']['value'])
        
        elevation = abs(math.degrees(targetAZEL['m1']['value']))
#        print elevation
#        exit()
#        elevation = 90. - dm.separation(zenith, target).get_value("deg")
        side_time=coords.mjd2lst(mjd2, position)
        return elevation, side_time/(3600.)
...
def runningAvg(xvals,yvals):
    bin_size = (max(xvals) - min(xvals)) / 20.
    bins = [(x*bin_size)+(min(xvals)) for x in range(20)]
    yavg=[]
    ysig=[]
    xbins_to_pop=[]
    count=0
    ...
    for xbin in range(len(bins_tmp)):
        ydata=[yvals[a] for a in range(len(yvals)) if xvals[a] >= bins_tmp[xbin] if xvals[a] < bins_tmp[xbin]+bin_size]
        if len(ydata) != 0:
...
def plotfig_scatter(data, xlabel, ylabel, plotname):
    xvals=[x[0] for x in data]
    yvals=[x[1] for x in data]
    plt.figure()
...
def extr_src_data(dataset_id):
    sources=[]
    data=open('ds_'+dataset_id+'_sources_fluxQC.csv','r')
    for lines in data:
        lines=lines.rstrip()
...
def extr_skymodel(skymodel):
    data=open(skymodel,'r')
    rows = data.readlines()
    data.close()
    srcs=[]
    srcsTmp=[]
